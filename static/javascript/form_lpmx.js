...
var FormLpmx = {
  ...
  exe: function (cmd) {
    switch (cmd) {
      ...
      case "select_text":
        ...
        break;
      case "save_data":
        if (confirm("Save Data ?")) this.save_data();
        break;
      case "load_data":
        if (confirm("Load Data ?")) this.load_data();
        break;
      case "check_text":
        this.check_text();
        break;
      ...
      case "show_store":
        ...
        break;
      case "clear_store":
        if (confirm("Clear Local Store ?")) DbFormLpmx.clear_store();
        break;
      case "close":
        cmd_close();
        break;
  ...
  check_text: function () {
    ...
    for (const k of fkos.sort()) {
      ...
      let ls = [];
      for (const row of rows) ls.push(row[1]);
      const s = ls.join("&nbsp;&nbsp;&nbsp;");
      const h = `<div>${s}</div>`;
      jt.append(h);
    }
    ...
    let fks = DbFormLpmx.form_lst.map((e) => e[1]);
    let fkes = fks.filter((e) => !tks.includes(e));
    for (const k of fkes) jt.append(`<div>${k}</div>`);
    jt.append("<hr/></div>");
    const h = jt.html();
...
var SelectText = {
  ...
  open: function (p_id, id, rows, call) {
    ...
    if (!this.wnd) this.wnd = UaWindowAdm.create(id, p_id);
